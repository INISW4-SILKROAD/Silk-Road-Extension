...
//       chrome.storage.local.set({ searchResults: data, lastFetchTime: Date.now() });
//     } else {
...
//     // 크롬 스토리지에서 결과를 가져옴
//     chrome.storage.local.get(['searchResults', 'lastFetchTime'], (result) => {
//       if (result.searchResults) {
//         console.log(`Data fetched after ${Date.now() - result.lastFetchTime} ms`);
...
//       // 검색 결과를 크롬 스토리지에 저장
//       chrome.storage.local.set({ searchResults: data.combinedResults, touchModelResponse: data.touchModelResponse, lastFetchTime: Date.now() });
//     } else {
//       console.error("이미지 링크 전송 실패");
//     }
//   } catch (error) {
...
//   if (message.action === 'fetchGoods') {
//     // 크롬 스토리지에서 결과를 가져옴
//     chrome.storage.local.get(['searchResults', 'touchModelResponse', 'lastFetchTime'], (result) => {
//       if (result.searchResults && result.touchModelResponse) {
//         console.log(`Data fetched after ${Date.now() - result.lastFetchTime} ms`);
//         sendResponse({ data: result.searchResults, touchModelResponse: result.touchModelResponse });
//       } else {
//         sendResponse({ error: 'No search results found' });
//       }
//     });
      ...
      while (1) switch (_context.prev = _context.next) {
        ...
        case 7:
          ...
          chrome.storage.local.set({
            searchResults: data.combinedResults,
            touchModelResponse: data.touchModelResponse,
        ...
        case 15:
          ...
          console.error("이미지 링크 전송 오류:", _context.t0);
        case 18:
        case "end":
...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  if (message.action === 'fetchGoods') {
    // 크롬 스토리지에서 결과를 가져옴
    chrome.storage.local.get(['searchResults', 'touchModelResponse', 'lastFetchTime'], function (result) {
      if (result.searchResults && result.touchModelResponse) {
        console.log("Data fetched after ".concat(Date.now() - result.lastFetchTime, " ms"));
        sendResponse({
          data: result.searchResults,
          touchModelResponse: result.touchModelResponse
        });
      } else {
        sendResponse({
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("860dbef03d6d99a7f194")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=background.9abc74797623f757f2c6.hot-update.js.map