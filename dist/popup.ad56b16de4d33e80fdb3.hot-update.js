...
app.post('/submit', /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(req, res) {
    var _req$body, imageUrl, pageUrl, flaskResponse, searchResults, gids, placeholders, query;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          _req$body = req.body, imageUrl = _req$body.imageUrl, pageUrl = _req$body.pageUrl;
          if (!(!imageUrl || !pageUrl)) {
            _context.next = 4;
            break;
          }
        ...
        case 4:
          console.log("Received image URL: ".concat(imageUrl));
          console.log("Received page URL: ".concat(pageUrl));
          _context.prev = 6;
          _context.next = 9;
          return axios.post('http://127.0.0.1:5002/search', {
            image_url: imageUrl
          });
        case 9:
          flaskResponse = _context.sent;
          searchResults = flaskResponse.data;
          console.log('Flask 서버로부터 받은 검색 결과:', searchResults);
          gids = searchResults.map(function (result) {
            return result.gid;
          });
          if (!(gids.length === 0)) {
            _context.next = 16;
          ...
          res.status(404).json({
            error: '검색 결과에서 일치하는 상품을 찾을 수 없음'
          });
          return _context.abrupt("return");
        case 16:
          placeholders = gids.map(function () {
            return '?';
          }).join(',');
          query = "SELECT * FROM service_goods WHERE ID IN (".concat(placeholders, ")");
          connection.query(query, gids, function (err, results) {
            if (err) {
              console.error('상품 불러오기 오류:', err);
              res.status(500).json({
                error: '상품 불러오기 오류'
            ...
            var combinedResults = searchResults.map(function (result) {
              return _objectSpread(_objectSpread({}, result), {}, {
                product: results.find(function (item) {
                  return item.ID === result.gid;
                }) || null
              });
            });
        ...
        case 21:
          ...
          console.error('Flask 서버로부터 검색 결과를 받는 중 오류 발생:', _context.t0);
          res.status(500).json({
            error: 'Flask 서버로부터 검색 결과를 받는 중 오류 발생'
          });
        case 25:
        case "end":
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("91f2f8e794be516d070d")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=popup.ad56b16de4d33e80fdb3.hot-update.js.map