...
//   socket.on('joinRoom', async (productId) => {
//     console.log(`Client joined room for product ${productId}`);
//     socket.join(productId);

//     if (productId) {
//       // JSON 파일에서 촉감 데이터를 읽어와 클라이언트에 전송
//       try {
//         const data = fs.readFileSync('./touch_data.json', 'utf-8');
//         const touchData = JSON.parse(data);
//         if (touchData.productId === productId) {
//           socket.emit('touchInfo', { productId, touchInfo: touchData.touchInfo });
//         }
//       } catch (err) {
